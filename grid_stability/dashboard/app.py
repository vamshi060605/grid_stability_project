...
def load_models():
    ...
    if not RF_PATH.exists() or not XGB_PATH.exists():
        st.error("System is not ready. Please contact the administrator.")
        st.stop()
...
def inject_fault(fault_type: str) -> dict | None:
    """Return fixed feature signatures — physically distinct per fault type."""
    SIGNATURES = {
        "line_outage":     {"vsi": 0.81, "rocov": 0.038, "thermal": 0.72, "fault_imp": 8.5},
        "load_surge":      {"vsi": 0.87, "rocov": 0.025, "thermal": 0.94, "fault_imp": 22.0},
        "generator_trip":  {"vsi": 0.74, "rocov": 0.048, "thermal": 0.65, "fault_imp": 12.0},
        "high_impedance":  {"vsi": 0.91, "rocov": 0.012, "thermal": 0.45, "fault_imp": 3.8},
    }
    return SIGNATURES.get(fault_type)
...
def main():
    ...
    if monitoring_mode == "Live Monitor":
        ...
        if "live_has_started" not in st.session_state:
            st.session_state.live_has_started = False
        ...
        fault_imp_val = st.sidebar.slider(
            ...
            value=st.session_state.get("fault_imp_val", 50.0), step=0.1, key="fault_imp_slider",
        ...
        default_params = (1.0, 0.0, 0.2, 50.0)
        if _current_params != default_params:
            st.session_state.user_has_interacted = True
        ...
        def manual_fault_callback(ftype, flabel):
            ...
            if injected:
                st.session_state.user_has_interacted = True
                st.session_state.active_fault = ftype
    ...
    vsi_val, rocov_val, thermal_val, fault_imp_val = 1.0, 0.0, 0.2, 50.0
    override = None
    X_df = None
    ...
    if monitoring_mode == "Manual Mode":
        ...
        # Mark live monitor as started after first successful reading
        if not st.session_state.live_has_started:
            st.session_state.live_has_started = True
            # First tick: show stable waiting state, skip prediction
            st.info("📡 Live Monitor is running — waiting for first grid reading...")
    ...
    with tab1:
        ...
        if not st.session_state.user_has_interacted:
            ...
            st.error("⚠️ Fault detected — see Fault Explainer tab for diagnosis and corrective actions.")
        ...
        if override is None and X_df is not None:
            ...
            with col_pred:
                ...
                st.metric("Secondary Model Confidence", f"{xgb_prob*100:.1f}%")
            with col_gauge:
            ...
            if _shap_weights is not None:
                ...
                with col_fsi:
                    st.info("Severity index is currently unavailable.")
    ...
    with tab2:
        ...
        if skip_panel2 or not st.session_state.user_has_interacted or pred_info is None or pred_info["label"] in ("STABLE", "RECOVERING"):
            ...
            if explanation is None:
                ...
                st.warning("Detailed explanation unavailable — showing simplified feature importance instead.")
    ...
    with tab3:
        ...
        if X_test_data is not None and y_test_data is not None:
            ...
            rf_prob = rf.predict_proba(X_test_data)[:, 1]
            rf_accuracy = accuracy_score(y_test_data, rf_pred)
            rf_f1 = f1_score(y_test_data, rf_pred, average='weighted')
            rf_auc = roc_auc_score(y_test_data, rf_prob)
            
            st.subheader("🌲 Random Forest")
            col1, col2, col3 = st.columns(3)
            col1.metric("Accuracy", f"{rf_accuracy*100:.1f}%")
            col2.metric("F1 Score", f"{rf_f1:.3f}")
            col3.metric("AUC-ROC",  f"{rf_auc:.3f}")

            # XGBoost Metrics
            xgb_pred = xgb.predict(X_test_data)
            xgb_prob = xgb.predict_proba(X_test_data)[:, 1]
            xgb_accuracy = accuracy_score(y_test_data, xgb_pred)
            xgb_f1 = f1_score(y_test_data, xgb_pred, average='weighted')
            xgb_auc = roc_auc_score(y_test_data, xgb_prob)

            st.subheader("⚡ XGBoost")
            col4, col5, col6 = st.columns(3)
            col4.metric("Accuracy", f"{xgb_accuracy*100:.1f}%")
            col5.metric("F1 Score", f"{xgb_f1:.3f}")
            col6.metric("AUC-ROC",  f"{xgb_auc:.3f}")
